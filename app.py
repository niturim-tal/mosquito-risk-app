...
# חלוקת המסך לתצוגה (2 עמודות)
        ...
        with col1:
            ...
            # 1. חישוב מרכז היישוב להתמרכזות אוטומטית
            try:
            ...
            if 'center_lat' in locals():
                m.location = [center_lat, center_lon]

            # 3. הגדרות ויזואליזציה מדויקות (החלפה ל-byte כדי להבטיח מיפוי ערכים בדיד)
            risk_img_byte = mosquito_risk_clipped.toByte()
            risk_vis = {
                'min': 1,
                'max': 4,
                'palette': ['00FF00', 'FFFF00', 'FFA500', 'FF0000']  # ירוק, צהוב, כתום, אדום
            }
            ...
            # 4. חילוץ שכבת הסיכון מ-GEE והוספתה למפה
            risk_map_id = ee.Image(risk_img_byte).getMapId(risk_vis)
            folium.TileLayer(
            ...
            # 5. חילוץ שכבת הטמפרטורה מ-GEE והוספתה למפה (כבוי כברירת מחדל)
            ...
            # 6. הוספת גבולות היישוב בקו שחור בולט
            try:
            ...
            # מקרא צבעים מפורט
            st.markdown("""
                <div style="display: flex; justify-content: space-around; background-color: #f8f9fa; padding: 10px; border-radius: 8px; font-size: 13px; margin-top: 10px; border: 1px solid #ddd;">
                    <div>🟢 <b>1:</b> סיכון נמוך (ירוק)</div>
                    <div>🟡 <b>2:</b> סיכון בינוני (צהוב)</div>
                    <div>🟠 <b>3:</b> סיכון גבוה (כתום)</div>
                    <div>🔴 <b>4:</b> סיכון קריטי (אדום)</div>
                </div>
            """, unsafe_allow_html=True)
        with col2:
